...
&emsp;&emsp;我在这里只给出Arduino相关的部分，51和ARM大同小异，你只需要去下面我们结合实际操作来学习。
...
&emsp;&emsp;这类函数有一个特点，就是都以`draw`开头，可以画各种点、线、形。
...
* 描述：画一个实心框，左上角的位置对应`x`/`y`，宽为`w`，高为`h`。框的一部分可能会超出屏幕边界。这个函数会用当前颜色（[setDrawColor](#setDrawColor)）来画，对于单色显示屏，颜色0表示清除像素，颜色1表示显示像素。
